# meta developer: Богосокрушительный 
import random
...
@loader.owner
def register(cb):
    cb(BogosokrushitelMod())


class BogosokrushitelMod(loader.Module):
    strings = {"name": "krushitel"}

    async def client_ready(self, client, db) -> None:
        self.db = db
        self.client = client

    async def bogkrushilacmd(self, message):
        '''задержка + шапка'''
        args = utils.get_args_raw(message)
        if not args:
            self.db.set(self.strings["name"], "state", False)
            await utils.answer(message, "<b>соси</b>")
            return
        await utils.answer(
            message,
            "<b>засоси\n\n",
